...
def _build_defib(db):
    ...
    plugin = MagicMock()
    plugin.rpc.getinfo.return_value = {"id": OUR_ID}
    cfg = Config(dry_run=True, daily_budget_sats=100_000,
...
def _defib_row_status(db):
    conn = db._get_connection()
    row = conn.execute(
        "SELECT status FROM rebalance_history "
        "WHERE rebalance_type = 'diagnostic' ORDER BY id DESC LIMIT 1"
    ).fetchone()
    return row["status"] if row else None


def test_defibrillator_preserves_block_rows(tmp_path):
    from modules.database import Database

    db = Database(os.path.join(tmp_path, "defib.db"), MagicMock())
    db.initialize()
    r = _build_defib(db)

    def fake_execute(candidate, rebalance_id=None, **kwargs):
        # The engine records the shared row as 'skipped' (governor block).
        db.update_rebalance_result(rebalance_id, "skipped",
                                   error_message="governor_block: PAUSED")
        return ExecutionResult(success=False,
                               error="governor_block: PAUSED")

    r._execute_candidate_v2 = fake_execute

    r.diagnostic_rebalance("200x1x0")

    # The engine's deliberate 'skipped' record survives — the defibrillator
    # must not re-poison it to 'failed'.
    assert _defib_row_status(db) == "skipped"


def test_defibrillator_still_marks_real_failures(tmp_path):
    from modules.database import Database

    db = Database(os.path.join(tmp_path, "defib2.db"), MagicMock())
    db.initialize()
    r = _build_defib(db)

    r._execute_candidate_v2 = MagicMock(return_value=ExecutionResult(
        success=False, error="no_route"))

    r.diagnostic_rebalance("200x1x0")

    assert _defib_row_status(db) == "failed"
