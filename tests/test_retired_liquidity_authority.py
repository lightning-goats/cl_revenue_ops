...
def test_retiring_module_roots_are_present_at_characterization_checkpoint():
    assert {name for name, path in RETIRED_MODULES.items() if path.is_file()} == set(RETIRED_MODULES)


def test_retired_rpc_and_option_families_are_nonempty():
    assert _registered_rpcs("revenue-lnplus-")
    assert _registered_rpcs("revenue-boltz-")
    assert _registered_rpcs("revenue-planner-")
    assert _option_literals("revenue-ops-lnplus-")
...
def test_direct_mutation_verbs_have_the_expected_owner():
    expected = {
        "lnplus": ("urllib.request.urlopen", "connect_peer", "fund_channel", "signmessage"),
