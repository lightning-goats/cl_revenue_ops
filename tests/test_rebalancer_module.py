...
class TestRebalancerPlannerCoordination:
    """Tests for rebalancer-planner coordination via pending-close interface."""

    def test_set_capacity_planner(self, mock_plugin, mock_database):
        """set_capacity_planner stores reference on rebalancer."""
        from modules.config import Config
        from modules.rebalancer import EVRebalancer
        ...
        r = EVRebalancer(mock_plugin, cfg, mock_database)
        planner = MagicMock()
        r.set_capacity_planner(planner)
        assert r._capacity_planner is planner

    def test_rebalancer_skips_pending_close_channels(self, mock_plugin, mock_database):
        ...
        # Set up a capacity planner that says 111x1x0 is pending close
        planner = MagicMock()
        planner.is_pending_close = MagicMock(side_effect=lambda cid: cid == "111x1x0")
        r.set_capacity_planner(planner)

        # Channel 111x1x0 is depleted (10% outbound) — would normally be a rebalance target
        # Channel 222x2x0 is overfull (90% outbound) — source
        mock_plugin.rpc.listpeerchannels.return_value = {
            "channels": [
                {
                    "short_channel_id": "111x1x0",
                    "peer_id": "02" + "aa" * 32,
                    "state": "CHANNELD_NORMAL",
                    "spendable_msat": 100_000_000,  # 10% - depleted
                    "receivable_msat": 900_000_000,
                    "total_msat": 1_000_000_000,
                    "fee_proportional_millionths": 500,
                },
                {
                    ...
                    "peer_id": "02" + "bb" * 32,
                    "state": "CHANNELD_NORMAL",
                    "spendable_msat": 900_000_000,  # 90% - source
                    "receivable_msat": 100_000_000,
                    "total_msat": 1_000_000_000,
                    "fee_proportional_millionths": 300,
                },
            ]
        ...
        candidates = r.find_rebalance_candidates()
        # 111x1x0 is depleted but pending close, so it should be excluded.
        # With no depleted channels left, there are no candidates.
        assert candidates == []

        # Verify the planner was consulted
        planner.is_pending_close.assert_any_call("111x1x0")
