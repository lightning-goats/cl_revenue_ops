...
def test_totals_delta_counts_policy_change_without_inventing_rebalance_cost():
    ...
    assert runner.totals_delta(before, after) == {
        ...
        "rebalance_delivered_msat": 0,
        "rebalance_completed_count": 0,
    }
...
def test_completed_smoke_attributes_only_bounded_native_rebalance_volume(
    ...
    monkeypatch.setattr(
        runner, "run_reconciled_traffic", lambda *_args, **_kwargs: [{
            "family": "lnd", "payer": "payer", "sink": "sink",
            "amount_sats": 50_000, "payment_hash": "hash",
            "payment": {"success": True},
        }]
    )

    block = runner.run_smoke(
        object(), replica=95, results_dir=tmp_path, rounds=1,
        amount_sats=50_000, pause_seconds=0,
        traffic_pattern="forward-pressure", traffic_family="lnd",
    )
...
def test_controlled_lnd_depletion_uses_exact_lnd_first_hops(monkeypatch, tmp_path):
    ...
    state = runner.prepare_controlled_depletion(
        object(), replica=10, results_dir=tmp_path,
...
def test_controlled_payer_depletion_adds_equal_reverse_earning_traffic(
    ...
    assert admissions == [{"amount_sats": 333_334}]
    fixture = state["controlled_depletion"]
    assert fixture["depleted_side"] == "payer"
    assert fixture["counterflow_amount_sats_per_controller"] == 1_000_000
...
def test_competition_image_pins_all_source_revisions():
    ...
    assert runner.IMAGE == "cl-revenue-ops-polar-clboss:2792d10"
    assert runner.EXPECTED_REVENUE_REVISION.startswith("2792d10")
