...
- Implement a separately reviewed recorded-price EV replay slice; v0 retains price evidence but cannot recreate or evaluate historical Askrene pricing.
  - **Delivered 2026-08-22** (commits `ddc994b`, `55b28c0`, `6fcbfe3`, `50e88bf`): the offline reader now recomputes the recorded-price sats-EV gate from each final-selected pair's captured `score_decomposition` via the new pure module `modules/rebalance_ev_model.py`, and byte-compares `{final_score_sats, beats_do_nothing}` against the recorded verdicts (`tools/rebalance_replay.py` output gains additive `ev_gate_matches` / `ev_gate_pairs_checked`; a gate divergence is mismatch exit 1, structurally invalid gate evidence is input error exit 2). Wire validation requires the full gate key set with pinned model version `v2-sats-ev` on final pairs only; generated-pair decompositions remain opaque. The engine does not call the model module — duplication is deliberate so drift produces a mismatch. Limitation unchanged: this verifies the gate arithmetic against RECORDED probability/fee/utilization inputs, not the historical Askrene quote. Evidence: focused matrix 622 passed; full functional suite 3704 passed (excl. supply-chain pins).
