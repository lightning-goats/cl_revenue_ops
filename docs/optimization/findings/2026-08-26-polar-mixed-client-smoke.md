...
| Fresh network 3 at 25 ppm | A third seven-node network was provisioned sequentially through the existing Polar MCP process; all twelve channels opened and its two-round CLN/LND forward/reverse baseline settled 8/8. Baseline attribution was 6 target / 2 CLN competitor / 0 LND competitor. After both competitors were gossip-verified at 25 ppm, LND mission control was reset and only the CLN payer was restarted through MCP. Separate ten-payment 50,000-sat forward windows settled 20/20: the 10-ppm target carried all 10 LND payments and 4/10 CLN payments, while the remaining six split 3/3 across the competitors. This is the second independent 25-ppm client-divergence result, not yet the required three-network acceptance. |
| Fresh broadcast-state sentinel bug | The first dry-run fee cycle on network 3 left all real target policies at 10 ppm and wrote no fee-change history, but fresh strategy rows exposed `last_broadcast_fee_ppm=0`. The existing repair handled only positive tracked values, so the database's zero/unknown sentinel survived even when live policy was known. Cycle and fee-state loaders now initialize any non-positive tracked sentinel from a positive live policy and persist it. A red/green fresh-row regression covers both state models; live Polar readback converged all four diagnostics to 10 ppm without `setchannel`. |
| Fresh network 3 rebalance hold | With the plugin paused and a temporary 1,000-sat evaluation budget, four pairs were considered after the 25-ppm traffic window. Real mixed-client return routes for 374,998 sats and 49,996 sats cost 18 and 5 sats. Their final sats-EV values were −18.126042 and −4.995780, so the engine selected neither at the unchanged zero hold margin. Execution count, reservation count, and rebalance spend remained zero; the budget returned to zero and reconciliation was clean. |
...
| Regression suite | Focused rebalance suites: 519 passed. Planner plus operator/reporting regressions: 94 passed. Pending/atomic/engine suites after the late-success cooldown fix: 128 passed. Fresh-sentinel fee-controller family: 396 passed. Successful-failover telemetry family: 110 passed. Final exact-pinned Python 3.11 full suite after the fresh-row fix: 3,728 passed, 5 environment-dependent skips, and 2 intentional expected failures. |
...
- **Fresh fee-state initialization:** zero is the persistence sentinel for an
  unknown broadcast policy, not evidence that a positive live policy is zero.
  Initialize both cycle and DTS/PID state from the live policy before applying
  dry-run proposal logic, and persist that repair.
