...
| CB-4 category budgets (exploration=`channel_open`, tactical=`boltz`) self-deplete by spent+reserved, floored at 0 | **verified (code confirmed) + fail-open sub-case FIXED (commit 3521a68)** | Depletion arithmetic itself verified: test_exploration_budget_reduced_by_open_spend_and_reservations, test_tactical_budget_reduced_by_boltz_spend_and_reservations; code confirmed `_apply_category_spend_remaining` capex_budget.py:679-693. **But the DB-error path fails open with no covering test — see Anomalies #1, concrete code evidence below.** |
...
**CB-4 fail-open sub-case FIXED in commit 3521a68** ("Fix CB-4: capex budgets fail
closed on DB error"). `_get_total_capex_by_channel` and `_get_spend_ledger_summary`
now return None on DB error (with a warning on the `cl-revenue-ops.capex_budget`
logger) instead of empty dicts; `compute_allocations` treats None as degraded and
zeroes ALL channel, exploration and tactical budgets for the cycle (spend denied),
exposing `CapexAllocations.db_degraded=True` on the module's status dataclass.
Regression coverage: tests/test_capex_budget.py::TestDbErrorFailsClosed (raising
capex mock → all budgets 0; raising spend-ledger mock → all budgets 0; healthy path
unchanged with db_degraded=False; fail-closed warning logged). Contract CB-4
amended (docs/audit/contracts/capex_budget.md); the fail-open Uncertainties bullet
removed there. The Gaps bullet "no test drives the DB-error branches" is now
closed. The sections below are retained as the historical evidence record.

## Fail-open finding (CB-4 sub-case) — concrete code evidence (historical; fixed in 3521a68)
