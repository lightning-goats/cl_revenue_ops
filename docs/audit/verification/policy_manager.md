...
| PM-I13 corrupt rows degrade to safe defaults | **open (refuted) → FIXED (commit a095a20)** | The three named guards are real: :362-373 (tags JSON → []), :388-396 (strategy → DYNAMIC), :398-406 (mode → ENABLED), all log-and-continue; no test feeds a corrupt row through _row_to_policy. REFUTED: only those 3 of the row's fields are guarded, and the invariant's conclusion "a corrupted row can neither brick policy reads nor silently freeze a peer" is false. `expires_at` passes through `_row_to_policy` raw (:383, :417); a corrupt TEXT value (SQLite columns are dynamically typed) reaches `is_expired()` → `int(time.time()) > 'garbage'` → TypeError (proven executable; ENABLE_AUTO_EXPIRY=True at :43 so the comparison is live). That call sits inside `_load_cache` at :352 with no try/except, and `get_policy` calls `_load_cache` unguarded — ONE corrupt `expires_at` row makes EVERY `get_policy`/`get_all_policies` call raise, for all peers: exactly the brick-the-reads failure mode the invariant exists to prevent. (`fee_ppm_target`/`updated_at` are also unguarded, with per-consumer blast radius.) The verified verdict generalized from the 3 guarded fields to the whole row. |

## Fix pass (2026-07-01)

Three findings fixed on branch worktree-agent-a64a0da203d04f560; table rows above
marked FIXED with commit hashes. Historical evidence retained as written.

- **PM-I2 FIXED (commit f87e9e2)**: `set_policies_batch` now enforces the same
  STATIC-requires-target check as `set_policy` during its validate-first pass —
  any STATIC entry resolving to a None target fails the whole batch with
  ValueError and nothing is persisted (matching existing fail-whole-batch
  semantics, PM-I10). The entrenching test
  test_policy_manager.py::TestPolicyBatchOperations::test_batch_update_applies_rebalance_modes
  was updated (its STATIC entry now carries fee_ppm_target=500), the shared
  conftest `sample_policy_updates` fixture's STATIC entry gained a target, and
  new tests assert the rejection (upsert_policies_batch never called) and the
  static-with-target (including 0) happy path. Contract PM-I2/PM-I10 amended.
- **PM-I13 FIXED (commit a095a20)**: `_load_cache` now isolates failures per
  row — a row that raises during decode or `is_expired()` (e.g. corrupt TEXT
  `expires_at` → TypeError) is skipped with a warning and only that peer
  degrades to the permissive default; all other peers' policies keep serving.
  Tests: TestCorruptRowIsolation (corrupt + valid row seeded → valid one served,
  warning logged; corrupt peer gets DYNAMIC/ENABLED default). Contract PM-I13
  amended.
- **PM-I1 FIXED (commit 2fcba6d)**: `PEER_ID_PATTERN` re-anchored from
  `^...$` to `\A[0-9a-fA-F]{66}\Z`, so 66-hex + trailing `\n` (and any
  wrong-length/non-hex input) is strictly rejected on all write paths. Tests:
  TestValidation::test_peer_id_trailing_newline_rejected /
  test_peer_id_wrong_length_rejected / test_peer_id_non_hex_rejected. Contract
  PM-I1 amended (anchoring + corrected test citation). Note: database.py keeps
  its own separate `PEER_ID_PATTERN` with `^...$` anchors (database.py:456);
  that layer is outside this module's scope and was already documented as
  performing no validation on these paths.
