...
| Behavior change | None. |
| Rollback | n/a. |
| Status | **met** — PR 9 SHIPPED 2026-07-13: 40 scenario classes (tests/conformance/scenarios/), every expected block generated BY the reference implementation or copied verbatim from goldens; standalone validator passes over all 41 payloads; regeneration byte-identical (pinned); generated coverage report (docs/refactor/phase0/conformance-coverage.md) maps scenarios to requirements/reason codes/rules; new conformance_case.v0 + ledger_event.v0 schemas; sanitized production lifecycle captured (first governed spend). Two conflict rules recorded as EXPLICIT documented-gap cases (open-vs-LN+, rebalance-vs-structural) — Phase G work items, visible in the report. |
